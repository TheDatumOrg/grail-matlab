function [Zexact, Ztop5, Ztop10, Ztop20, Z99per, Z98per, Z97per, Z95per, Z90per, Z85per, Z80per, DistComp, RuntimeNystrom, RuntimeFD]=RepLearnFINALSINKComp(X, Dictionary, gamma, coeffs)
% Input
% X: original data (nxm)
...
for i=1:nrowsDic
    for j=1:nrowsDic
        W(i,j) = SINKCompressed(Dictionary(i,:),Dictionary(j,:),gamma,coeffs);
        DistComp = DistComp + 1;
    end    
end
...
for i=1:nrowsX
    disp(i);
       for j=1:nrowsDic
           E(i,j) = SINKCompressed(X(i,:),Dictionary(j,:),gamma,coeffs);
           DistComp = DistComp + 1;
       end    
end
