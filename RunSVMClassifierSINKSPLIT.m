function RunSVMClassifierSINKSPLIT(DataSetStartIndex, DataSetEndIndex)  
    ...
    for i = 1:length(Datasets)
            ...
            if (i>=DataSetStartIndex && i<=DataSetEndIndex)
                    ...
                    KM = dlmread( strcat( 'KernelMatricesSINKSPLIT/',char(Datasets(i)),'/', char(Datasets(i)), '_SINK_Sigma_', num2str(Thebestgamma3) ,'_TRAIN.kernelmatrix') );
                    ...
                    KM = dlmread( strcat( 'KernelMatricesSINKSPLIT/',char(Datasets(i)),'/', char(Datasets(i)), '_SINK_Sigma_', num2str(Thebestgamma3) ,'_TESTTOTRAIN.kernelmatrix') );
                    tic;
                    ...
                    dlmwrite( strcat('RunSVMClassifierSINKSPLIT/','RunSVMClassifierSINKSPLIT_', num2str(i) ), Results, 'delimiter', '\t');
...
function [Thebestgamma,Thebestcost,Thebestacc,Thebestiming] = GridSearchSVM1(GridStart,GridStep,GridEnd,TrainInstancesCount,TrainClassLabels,Datasets,DatasetsNumber)
                    ...
                    besttiming = zeros(1,length(log2cTmp));
                      ...
                      KM = dlmread( strcat( 'KernelMatricesSINKSPLIT/',char(Datasets(DatasetsNumber)),'/', char(Datasets(DatasetsNumber)), '_SINK_Sigma_', num2str(WarpValues(gamma)) ,'_TRAIN.kernelmatrix') );
...
function [Thebestgamma,Thebestcost,Thebestacc,Thebestiming] = GridSearchSVM2(GridStart,GridStep,GridEnd,TrainInstancesCount,TrainClassLabels,Datasets,DatasetsNumber,gamma)
                    ...
                    besttiming = zeros(1,length(log2cTmp));
                      ...
                      KM = dlmread( strcat( 'KernelMatricesSINKSPLIT/',char(Datasets(DatasetsNumber)),'/', char(Datasets(DatasetsNumber)), '_SINK_Sigma_', num2str(gamma) ,'_TRAIN.kernelmatrix') );
